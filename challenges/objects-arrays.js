...
graduates.forEach(grad => {
  let uniList = grad.university;
  universities.push(uniList);
});
...
const contactInfo = [];
graduates.map(grad => {
  let info = `${grad.first_name} ${grad.email}`;
...
zooAnimals.forEach(animal => {
  let bothNames = `Name: ${animal.animal_name}, Scientific Name: ${
    animal.scientific_name
  }`;
  animalNames.push(bothNames);
});
...
zooAnimals.map(animal => {
  let firstName = animal.animal_name;
  lowerCase.push(firstName.toLowerCase());
});
...
*/
let lowerPopulation = [];
const urgentPop = animal => {
  return animal.population < 5;
};
lowerPopulation = zooAnimals.filter(urgentPop);
...
*/
const populationTotal = zooAnimals.reduce((pop, arr) => {
  return pop + arr.population;
}, 0);
